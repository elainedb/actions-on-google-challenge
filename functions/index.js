...
exports.factsAboutGoogle = functions.https.onRequest((request, response) => {
  ...
  function unhandledDeepLinks (app) {
    if (app.hasSurfaceCapability(app.SurfaceCapabilities.SCREEN_OUTPUT)) {
      ...
      app.ask(`Welcome to Facts about Google! I'd really rather not talk about ${app.getRawInput()}. Wouldn't you rather talk about Google? I can tell you about Google's history or its headquarters. Which do you want to hear about?`,
        NO_INPUTS);
    }
  }
  ...
  function tellFact (app) {
    ...
    if (factCategory === FACT_TYPE.HISTORY) {
      ...
      if (app.hasSurfaceCapability(app.SurfaceCapabilities.SCREEN_OUTPUT)) {
        app.ask(app.buildRichResponse()
          .addSimpleResponse(`Sorry, I didn't understand. I can tell you about Google's history, or its  headquarters. Which one do you want to hear about?`)
          .addSuggestions(['History', 'Headquarters']));
      } else {
  ...
  function noFactsLeft (app, currentCategory, redirectCategory) {
    ...
    let response = `Looks like you've heard all there is to know about the ${currentCategory} of Google. I could tell you about its ${redirectCategory} instead. `;
    if (!app.data.catFacts || app.data.catFacts.length > 0) {
